...
public class RippleView extends RelativeLayout {
    ...
    private int timer = 0;/*外部圈*/
    private int timerEmpty = 0; /*两个圈的时候 内部圈*/
    ...
    public void draw(Canvas canvas) {
        ...
        if (animationRunning) {
            ...
            /*rippleType == 1 double*/
            if (rippleType == 1 && getOriginBitmap() != null && (((float) timer * frameRate) / rippleDuration) > 0.4f) {
                /*动画剩余的时间*/
                if (durationEmpty == -1)
    ...
    private float getCurrentRadius(){
        ...
        return (radiusMax * (((float) timer * frameRate) / rippleDuration));
    }
    ...
    private int getPaintAlpha(){
        ...
        if (rippleType == 1) {
            if ((((float) timer * frameRate) / rippleDuration) > 0.6f)
               return (int) (rippleAlpha - ((rippleAlpha) * (((float) timerEmpty * frameRate) / (durationEmpty))));
            else
                return rippleAlpha;
        }

        return (int) (rippleAlpha - ((rippleAlpha) * (((float) timer * frameRate) / rippleDuration)));
    }
